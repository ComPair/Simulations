...
void PlotBackground(){
	...
	TFile *f = new TFile("HadronicBackground/HadronicBackground.R1.spectrum.root");
	TCanvas * C = (TCanvas*)f->Get("c1");
	TH1D * h_had = (TH1D*)C->GetPrimitive("EnergySpectrum");

	f = new TFile("LeptonicBackground/LeptonicBackground.R1.spectrum.root");
	C = (TCanvas*)f->Get("c1");
	TH1D * h_lep = (TH1D*)C->GetPrimitive("EnergySpectrum");

	f = new TFile("PhotonicBackground/PhotonicBackground.R1.spectrum.root");
	C = (TCanvas*)f->Get("c1");
	TH1D * h_pho = (TH1D*)C->GetPrimitive("EnergySpectrum");

	f = new TFile("TrappedHadronicBackground/TrappedHadronicBackground.R1.spectrum.root");
	C = (TCanvas*)f->Get("c1");
	TH1D * h_thad = (TH1D*)C->GetPrimitive("EnergySpectrum");

	f = new TFile("TrappedLeptonicBackground/TrappedLeptonicBackground.R1.spectrum.root");
	C = (TCanvas*)f->Get("c1");
	TH1D * h_tlep = (TH1D*)C->GetPrimitive("EnergySpectrum");

	f = new TFile("HadronicActivation/HadronicActivation.R1.spectrum.root");
	C = (TCanvas*)f->Get("c1");
	TH1D * h_had_decay = (TH1D*)C->GetPrimitive("EnergySpectrum");

	f = new TFile("TrappedHadronicActivation/TrappedHadronicBackgroundDecay.R1.spectrum.root");
	C = (TCanvas*)f->Get("c1");
	...
	//Simulations were for 7200 seconds, so divide by time
	h_had->Scale(1./7200);
